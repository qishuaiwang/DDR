...
`ifndef GUARD_SVT_DFI_LPDDR4_WR_SEQ_SV
`define GUARD_SVT_DFI_LPDDR4_WR_SEQ_SV

class svt_dfi_lpddr4_wr_seq  extends uvm_sequence#(svt_dfi_mc_lpddr4_transaction);
 
  `uvm_object_utils(svt_dfi_lpddr4_wr_seq )
  ...
  rand reg [8*`DWC_DDRPHY_NUM_DBYTES-1:0] data[16];
  rand reg [`DWC_DDRPHY_NUM_DBYTES-1:0] dfi_wrdata_mask[16];
 rand bit lp4_otf_bl;//LP4 spec MR1[1:0] == 2
  ...
  virtual task pre_body();
    uvm_phase phase;
    super.pre_body();
    `svt_trace("svt_dfi_lpddr4_wr_seq ","Starting of the sequence : svt_dfi_lpddr4_wr_seq_1");
  ...
  virtual task post_body();
    uvm_phase phase;
    super.post_body();
    `svt_trace("svt_dfi_lpddr4_wr_seq ","Starting of the sequence : svt_dfi_lpddr4_wr_seq_2");
    `ifdef SVT_UVM_12_OR_HIGHER
       phase = get_starting_phase();
    `else
       phase = starting_phase;
    ...
    if (phase!=null) begin
      ...
      `svt_debug("svt_dfi_lpddr4_wr_seq ","Dropped all objections");
    end
  endtask: post_body

  virtual task body();
    `svt_trace("svt_dfi_lpddr4_wr_seq ","Starting of the sequence : svt_dfi_lpddr4_wr_seq ");
    p_sequencer.get_host_cfg(get_cfg);
    if (!$cast(dfi_config, get_cfg)) begin
      `svt_fatal("svt_dfi_lpddr4_wr_seq ", "Unable to $cast the configuration to a svt_host_agent_configuration class");
    end
    ...
    case(cfg.PState)
      0: 
         `ifdef WRITE_DM0
         drive_seq.COMMAND =svt_dfi_drive_sequence::MWR_ONE;
         `else
...
//          $display ("debug for data[%0d]=%h ,dfi_wrdata_mask[%0d]=%b ,drive_seq.DATA[%0d]=%h ,drive_seq.DM[%0d]=%b",brst,data[brst][((i+1)*8-1)-:8],brst,dfi_wrdata_mask[brst][i-:1],brst,drive_seq.DATA[brst][((i+1)*8-1)-:8],brst,drive_seq.DM[brst][i-:1]);
    ...
    `svt_trace("svt_dfi_lpddr4_wr_seq ","Ending of the sequence : svt_dfi_lpddr4_wr_seq ");
   endtask
    
endclass: svt_dfi_lpddr4_wr_seq 
`endif
//-----------END OF LINE -------------------------------------------//
